...
async def generate_graph(guild: discord.Guild, history: List[Thank], format="gif") -> str:
    ...
    print(len(COLORS))
    lines: Dict[int, Line2D] = {u: plt.plot([], [], '-', color=f"xkcd:{COLORS[(i*3)%len(COLORS)]}")[0] for i, u in enumerate(user_id_historys.keys())}
    ...
    ani = FuncAnimation(fig, update, np.linspace(0, total_days+PAUSE_FRAMES, total_days+PAUSE_FRAMES+1), init_func=init, repeat=False)
    ...
    with open(filename, "rb") as file:
        data = file.read()
    data_no_rep = data.replace(b"NETSCAPE2.0\x03\x01\x00\x00", b"NETSCAPE2.0\x03\x01\x00\x01")
    with open(filename, "wb") as file:
        file.write(data_no_rep)
    return filename
